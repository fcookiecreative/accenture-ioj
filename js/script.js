jQuery(function () {
  ...
  const $carouselContent = jQuery('.ioj-carousel-content');
  const $mobileNavBar = jQuery('.ioj-nav--mobile');
  const $journeyParts = jQuery('.ioj-journey-svg__g');
  /* JOURNEY PARTS */
  $journeyParts.on('mouseenter', function () {
    $journeyParts.closest('.ioj-journey-part').filter('.active').removeClass('active');
    jQuery(this).closest('.ioj-journey-part').addClass('active');
  });
  $journeyParts.on('mouseleave', function () {
    $journeyParts.closest('.ioj-journey-part').filter('.active').removeClass('active');
    // Add .active for all journey part
    //$journeyParts.closest('.ioj-journey-part').addClass('active');
  });
  /* NAV BUTTONS CLICKS */
